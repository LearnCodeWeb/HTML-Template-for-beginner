<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="">
    <meta name="author" content="">
    <title>::.. Test Template ..::</title>

    <!-- Bootstrap core CSS -->
    <link href="assets/bootstrap/css/bootstrap.min.css" rel="stylesheet">
	
    <!-- Just for debugging purposes. Don't actually copy this line! -->
    <!--[if lt IE 9]><script src="themes/assets/js/ie8-responsive-file-warning.js"></script><![endif]-->

    <!-- HTML5 shim and Respond.js IE8 support of HTML5 elements and media queries -->
    <!--[if lt IE 9]>
      <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
      <script src="https://oss.maxcdn.com/libs/respond.js/1.4.2/respond.min.js"></script>
    <![endif]-->

	<!-- CSS Implementing Plugins -->
    <link rel="stylesheet" href="assets/custom/css/flexslider.css" type="text/css" media="screen">    	
    <link rel="stylesheet" href="assets/custom/css/parallax-slider.css" type="text/css">
    <link rel="stylesheet" href="assets/font-awesome-4.1.0/css/font-awesome.min.css" type="text/css">
    <link rel="stylesheet" href="assets/custom/css/carousel.css" media="all" type="text/css">
    <link rel="stylesheet" href="sass/css/oceanBraineffect.css" media="all" type="text/css">

    <!-- Custom styles for this template -->
	
    <link href="assets/custom/css/oceanbrains.css" rel="stylesheet">
    <link rel="shortcut icon" href="assets/custom/img/icons/icon.ico">
  </head>
<!-- NAVBAR
================================================== -->
  <body>
	
    <!-- topHeaderSection -->	
    <div class="topHeaderSection">
    <div class="header">			
		<div class="container">
        <div class="col-sm-12" id="_top_links">
            <div class="pull-right">
            	<strong><span class="fa fa-phone-square"></span>&nbsp;+92 324 4767220</strong>
                <strong><span class="fa fa-skype"></span>&nbsp;HereID</strong>
                <strong>Follow Us:
            	<a href="#"><span class="fa fa-facebook-square"></span></a>
                <a href="#"><span class="fa fa-google-plus-square"></span></a>
                <a href="#"><span class="fa fa-linkedin-square"></span></a>
                <a href="#"><span class="fa fa-twitter-square"></span></a>
                </strong>
            </div>
            <div class="clearfix"></div>
        </div>
        <a class="navbar-brand" href="index.html"><img src="assets/custom/img/logo.png" class="img-responsive" alt="OceanBrains PVT Limited (LOGO)" /></a>
        <div class="navbar-header">
          <button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
            <span class="sr-only">Toggle navigation</span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
          </button>
        </div>
        <div class="navbar-collapse collapse">
          <ul class="nav navbar-nav navbar-right">
            <li><a href="index.php"><span class="fa fa-home"></span>&nbsp;Home</a></li>
            <li class="active"><a href="about.php"><span class="fa fa-user"></span>&nbsp;About us</a></li>
            <li class="dropdown">
			  <a href="#" class="dropdown-toggle" data-toggle="dropdown"><span class="fa fa-book"></span>&nbsp;Services <b class="caret"></b></a>
              <ul class="dropdown-menu">
                <li><a title="Web Development" href="webdevelopment.php">Web Development</a></li>
                <li><a title="Mobile Development" href="mobiledevelopment.php">Mobile Development</a></li>
                <li><a title="Website Designing" href="webdesign.php">Website Designing</a></li>
                <li><a title="Logo Designing" href="logodesign.php">Logo Designing</a></li>
                <li><a title="Broucher Designing" href="brochuresdesign.php">Broucher Designing</a></li>
                <li><a title="SEO Services" href="seo.php">SEO Services</a></li>
              </ul>
            </li>
            
            <li><a href="portfolio.php"><span class="fa fa-folder"></span>&nbsp;Portfolio</a></li>
            <li><a href="blog.php"><span class="fa fa-rss-square"></span>&nbsp;Blog</a></li>
            <li><a href="contact.php"><span class="fa fa-envelope"></span>&nbsp;Contact</a></li>
          </ul>
        </div><!--/.nav-collapse -->
      </div>
	</div>
	</div>

<!--    <!-- bannerSection --
    <div class="bannerSection" style="background:url(assets/custom/img/slide/slide-bg21.jpg);">
		<div class="slider-inner">
			<div id="da-slider" class="da-slider">
				<div class="da-slide">
					<h2><i>We build</i> <br> <span class="margin_one">RESPONSIVE</span> <br> <i>Web Designs</i></h2>
					<p><i>Professional look</i> <br> <i class="margin_three">Compitable in different device</i> <br> <i>Mordern technology in use</i></p>
					<div class="da-img"><img src="assets/custom/img/slide/responive_webdesign.png" alt="" class="img-rounded" /></div>
				</div>
                
                <div class="da-slide">
					<h2>We Provide <br> <i class="margin_one" style="color:#933;"> Many Services</i> <br> <i>Web markiting, seo</i></h2>
                    <p>
                        <i>CHECK UR SERVICES ON</i><br>
                        <a href="#"><span class="fa fa-2x fa-facebook-square"></span></a>
                        <a href="#"><span class="fa fa-2x fa-google-plus-square"></span></a>
                        <a href="#"><span class="fa fa-2x fa-linkedin-square"></span></a>
                        <a href="#"><span class="fa fa-2x fa-twitter-square"></span></a>
                    </p>

					<div class="da-img"><img src="assets/custom/img/slide/growBanner1.png" alt="" class="img-rounded" /></div>
				</div>
                
                <div class="da-slide">
					<h2><i>We build</i> <br> <span class="margin_one">RESPONSIVE</span> <br> <i>Web Designs</i></h2>
					<p><i>Professional look</i> <br> <i class="margin_three">Compitable in different device</i> <br> <i>Mordern technology in use</i></p>
					<div class="da-img"><img src="assets/custom/img/slide/webdesigns.png" alt="" class="img-rounded" /></div>
				</div>
				

				<nav class="da-arrows">
					<span class="da-arrows-prev"></span>
					<span class="da-arrows-next"></span>		
				</nav>
			</div><!--/da-slider--
		</div><!--/slider--
		<!--=== End Slider ===--

	</div>-->


	<div class="container">

        <div class="row">

            <div class="col-lg-12">
                <h1 class="page-header">About
                    <small>It's Nice to Meet You!</small>
                </h1>
                <ol class="breadcrumb">
                    <li><a href="index.php">Home</a>
                    </li>
                    <li class="active">About</li>
                </ol>
            </div>

        </div>

        <div class="row">
			<div class="col-lg-12">
            	<div class="col-md-6" id="main" rol="main">
            <!-- Carousel
                    ================================================== -->
                    <div id="myCarousel" class="carousel slide" data-ride="carousel">
                      <!-- Indicators -->
                      <div class="carousel-inner">
                        <div class="item">
                          <img src="assets/custom/img/portfolio/webdesign/1363775228.jpg" class="img-responsive" alt="Second slide">
                        </div>
                        <div class="item">
                          <img src="assets/custom/img/portfolio/webdesign/1376546644.jpg" class="img-responsive" alt="First slide">
                        </div>
                        <div class="item">
                          <img src="assets/custom/img/portfolio/webdesign/mind-grids-img.jpg" class="img-responsive" alt="First slide">
                        </div>
                        <div class="item active">
                          <img src="assets/custom/img/portfolio/webdesign/1376546605.jpg" class="img-responsive" alt="Third slide">
                        </div>
                      </div>
					  <!--<a href="#myCarousel" data-slide="prev" class="btn btn-warning" ><span class="fa fa-chevron-left"></span></a>
                      <a href="#myCarousel" data-slide="next" class="btn btn-warning" ><span class="fa fa-chevron-right"></span></a>-->
            </div>
            <!-- /.carousel -->
                    
            </div>
            	<div class="col-md-6" id="aboutOcB">
                <h2>Welcome to 'OceanBrains'</h2>
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam viverra euismod odio, gravida pellentesque urna varius vitae. Sed dui lorem, adipiscing in adipiscing et, interdum nec metus. Mauris ultricies, justo eu convallis placerat, felis enim.</p>
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam viverra euismod odio, gravida pellentesque urna varius vitae. Sed dui lorem, adipiscing in adipiscing et, interdum nec metus. Mauris ultricies, justo eu convallis placerat, felis enim.</p>
            </div>
			</div>
        </div>

        <!-- Team Member Profiles -->

        <div class="row" id="main" rol="main">

            <div class="col-lg-12">
                <h1 class="page-header">Our Team</h1>
            </div>

            <div class="col-sm-4">
                <img class="img-responsive" src="http://placehold.it/750x450">
                <h3>Muhammad Raheel
                    <small>Sr. PHP Developer</small>
                </h3>
                	<p>
                		What does this team member do? Keep it short! This is also a great spot for social links!
                    </p>
                <ul class="list-unstyled list-inline list-social-icons">
                    <li class="tooltip-social facebook-link"><a href="#facebook-profile" data-toggle="tooltip" data-placement="top" title="Facebook"><i class="fa fa-facebook-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social linkedin-link"><a href="#linkedin-profile" data-toggle="tooltip" data-placement="top" title="LinkedIn"><i class="fa fa-linkedin-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social twitter-link"><a href="#twitter-profile" data-toggle="tooltip" data-placement="top" title="Twitter"><i class="fa fa-twitter-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social google-plus-link"><a href="#google-plus-profile" data-toggle="tooltip" data-placement="top" title="Google+"><i class="fa fa-google-plus-square fa-2x"></i></a>
                    </li>
                </ul>
            </div>

            <div class="col-sm-4">
                <img class="img-responsive" src="http://placehold.it/750x450">
                <h3>Zeeshan Gill
                    <small>PHP Developer</small>
                </h3>
                <p>What does this team member do? Keep it short! This is also a great spot for social links!</p>
                <ul class="list-unstyled list-inline list-social-icons">
                    <li class="tooltip-social facebook-link"><a href="#facebook-profile" data-toggle="tooltip" data-placement="top" title="Facebook"><i class="fa fa-facebook-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social linkedin-link"><a href="#linkedin-profile" data-toggle="tooltip" data-placement="top" title="LinkedIn"><i class="fa fa-linkedin-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social twitter-link"><a href="#twitter-profile" data-toggle="tooltip" data-placement="top" title="Twitter"><i class="fa fa-twitter-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social google-plus-link"><a href="#google-plus-profile" data-toggle="tooltip" data-placement="top" title="Google+"><i class="fa fa-google-plus-square fa-2x"></i></a>
                    </li>
                </ul>
            </div>

            <div class="col-sm-4">
                <img class="img-responsive" src="http://placehold.it/750x450">
                <h3>Hassam Mubarik
                    <small>Junior PHP Developer</small>
                </h3>
                <p>What does this team member do? Keep it short! This is also a great spot for social links!</p>
                <ul class="list-unstyled list-inline list-social-icons">
                    <li class="tooltip-social facebook-link"><a href="#facebook-profile" data-toggle="tooltip" data-placement="top" title="Facebook"><i class="fa fa-facebook-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social linkedin-link"><a href="#linkedin-profile" data-toggle="tooltip" data-placement="top" title="LinkedIn"><i class="fa fa-linkedin-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social twitter-link"><a href="#twitter-profile" data-toggle="tooltip" data-placement="top" title="Twitter"><i class="fa fa-twitter-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social google-plus-link"><a href="#google-plus-profile" data-toggle="tooltip" data-placement="top" title="Google+"><i class="fa fa-google-plus-square fa-2x"></i></a>
                    </li>
                </ul>
            </div>

            <div class="col-sm-4">
                <img class="img-responsive" src="http://placehold.it/750x450">
                <h3>Zaid Bin Khalid
                    <small>Web Designer & PHP Developer</small>
                </h3>
                <p>What does this team member do? Keep it short! This is also a great spot for social links!</p>
                <ul class="list-unstyled list-inline list-social-icons">
                    <li class="tooltip-social facebook-link"><a href="#facebook-profile" data-toggle="tooltip" data-placement="top" title="Facebook"><i class="fa fa-facebook-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social linkedin-link"><a href="#linkedin-profile" data-toggle="tooltip" data-placement="top" title="LinkedIn"><i class="fa fa-linkedin-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social twitter-link"><a href="#twitter-profile" data-toggle="tooltip" data-placement="top" title="Twitter"><i class="fa fa-twitter-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social google-plus-link"><a href="#google-plus-profile" data-toggle="tooltip" data-placement="top" title="Google+"><i class="fa fa-google-plus-square fa-2x"></i></a>
                    </li>
                </ul>
            </div>

            <div class="col-sm-4">
                <img class="img-responsive" src="http://placehold.it/750x450">
                <h3>Lubna Yaqoob
                    <small>SQA Engineer</small>
                </h3>
                <p>What does this team member do? Keep it short! This is also a great spot for social links!</p>
                <ul class="list-unstyled list-inline list-social-icons">
                    <li class="tooltip-social facebook-link"><a href="#facebook-profile" data-toggle="tooltip" data-placement="top" title="Facebook"><i class="fa fa-facebook-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social linkedin-link"><a href="#linkedin-profile" data-toggle="tooltip" data-placement="top" title="LinkedIn"><i class="fa fa-linkedin-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social twitter-link"><a href="#twitter-profile" data-toggle="tooltip" data-placement="top" title="Twitter"><i class="fa fa-twitter-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social google-plus-link"><a href="#google-plus-profile" data-toggle="tooltip" data-placement="top" title="Google+"><i class="fa fa-google-plus-square fa-2x"></i></a>
                    </li>
                </ul>
            </div>

            <div class="col-sm-4">
                <img class="img-responsive" src="http://placehold.it/750x450">
                <h3>Junaid Iqbal
                    <small>Project Manager</small>
                </h3>
                <p>What does this team member do? Keep it short! This is also a great spot for social links!</p>
                <ul class="list-unstyled list-inline list-social-icons">
                    <li class="tooltip-social facebook-link"><a href="#facebook-profile" data-toggle="tooltip" data-placement="top" title="Facebook"><i class="fa fa-facebook-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social linkedin-link"><a href="#linkedin-profile" data-toggle="tooltip" data-placement="top" title="LinkedIn"><i class="fa fa-linkedin-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social twitter-link"><a href="#twitter-profile" data-toggle="tooltip" data-placement="top" title="Twitter"><i class="fa fa-twitter-square fa-2x"></i></a>
                    </li>
                    <li class="tooltip-social google-plus-link"><a href="#google-plus-profile" data-toggle="tooltip" data-placement="top" title="Google+"><i class="fa fa-google-plus-square fa-2x"></i></a>
                    </li>
                </ul>
            </div>

        </div>

        <!-- Our Clients -->

        <div class="row">

            <div class="col-lg-12">
                <h1 class="page-header">Our Clients</h1>
            </div>

            <div class="col-md-2 col-sm-4 col-xs-6">
                <img class="img-responsive img-customer" src="http://placehold.it/500x300">
            </div>

            <div class="col-md-2 col-sm-4 col-xs-6">
                <img class="img-responsive img-customer" src="http://placehold.it/500x300">
            </div>

            <div class="col-md-2 col-sm-4 col-xs-6">
                <img class="img-responsive img-customer" src="http://placehold.it/500x300">
            </div>

            <div class="col-md-2 col-sm-4 col-xs-6">
                <img class="img-responsive img-customer" src="http://placehold.it/500x300">
            </div>

            <div class="col-md-2 col-sm-4 col-xs-6">
                <img class="img-responsive img-customer" src="http://placehold.it/500x300">
            </div>

            <div class="col-md-2 col-sm-4 col-xs-6">
                <img class="img-responsive img-customer" src="http://placehold.it/500x300">
            </div>

        </div>

    </div>
    <!-- /.container -->

                <div class="clearfix"></div>
	
	<br>
    <!-- footerTopSection -->
    <div class="footerTopSection">
            <div class="footer">
                <div class="row">
                  <div class="col-md-3">
                    <h3>Quick Links</h3>
                    <a href="#">
                    	<span class="fa fa-2x fa-home pull-left fb_color"></span>
                        <div class="col-lg-4">Home</div>
                    </a><br>
                    <hr class="clearfix">
                    <a href="#">
                    	<span class="fa fa-2x fa-user pull-left"></span>
                        <div class="col-lg-4">About Us</div>
                    </a><br>
                    <hr class="clearfix">
                    <a href="#">
                    	<span class="fa fa-2x fa-book pull-left link_color"></span>
                        <div class="col-lg-4">Services</div>
                    </a><br>
                    <hr class="clearfix">
                    <a href="#">
                    	<span class="fa fa-2x fa-folder pull-left orng_color"></span>
                        <div class="col-lg-4">Portfolio</div>
                    </a><br>
                    <hr class="clearfix">
                    <a href="#">
                    	<span class="fa fa-2x fa-shield pull-left go_color"></span>
                        <div class="col-lg-4">Privacy</div>
                    </a><br>
                    <hr class="clearfix">
                    <a href="#">
                    	<span class="fa fa-2x fa-unlock-alt pull-left gren_color"></span>
                        <div class="col-lg-4">Tearms</div>
                    </a><br>
                  </div>
                  <div class="col-md-3">
                    <h3>Contact Information</h3>
                    <p>
                        Name<br>
                        Address<br>
                        <strong>Phone Number</strong><br>
                        xxxx-xxxxxx<br>
                        <strong>Direct</strong>
                        xxxx-xxxxxx<br>
                        <strong>FAX</strong>
                        xxxx-xxxxxx<br>
                        <strong>Email</strong> :  info@example.com
                        
                    </p>
                  </div>
                  <div class="col-md-3">
                    <h3>Fallow us on</h3>
                        <a href="#">
                        	<span class="fa fa-2x fa-facebook-square pull-left"></span>
                            <div class="col-lg-4">FaceBook</div>
                        </a><br>
                        <hr class="clearfix">
                        <a href="#">
                        	<span class="fa fa-2x fa-linkedin-square pull-left"></span>
                            <div class="col-lg-4">LinkedIn</div>
                        </a><br>
                        <hr class="clearfix">
                        <a href="#">
                        	<span class="fa fa-2x fa-twitter-square pull-left"></span>
                            <div class="col-lg-4">Twitter</div>
                        </a><br>
                        <hr class="clearfix">
                        <a href="#">
                        	<span class="fa fa-2x fa-google-plus-square pull-left"></span>
                            <div class="col-lg-4">Google+</div>
                        </a><br>
                    
                  </div>
                  <div class="col-md-3">
                    <h3>We Provide</h3>
                        <div class="panel-group" id="accordion">
                          <div class="panel panel-default">
                            <div class="panel-heading">
                              <h4 class="panel-title">
                                <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
                                  Heading Here
                                </a>
                              </h4>
                            </div>
                            <div id="collapseOne" class="panel-collapse collapse in">
                              <div class="panel-body">
                                The key to be a successful software developer is to think outside the box– well, think outside the box you'r in.
                              </div>
                            </div>
                          </div>
                          <div class="panel panel-default">
                            <div class="panel-heading">
                              <h4 class="panel-title">
                                <a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo">
                                  Heading Here
                                </a>
                              </h4>
                            </div>
                            <div id="collapseTwo" class="panel-collapse collapse">
                              <div class="panel-body">
                               The key to be a successful software developer is to think outside the box– well, think outside the box you'r in.
                              </div>
                            </div>
                          </div>
                          <div class="panel panel-default">
                            <div class="panel-heading">
                              <h4 class="panel-title">
                                <a data-toggle="collapse" data-parent="#accordion" href="#collapseThree">
                                  Heading Here
                                </a>
                              </h4>
                            </div>
                            <div id="collapseThree" class="panel-collapse collapse">
                              <div class="panel-body">
                                The key to be a successful software developer is to think outside the box– well, think outside the box you'r in.
                              </div>
                            </div>
                          </div>
                        </div>
    
                  </div> <!--col-md-3 end-->
                  
                </div>
                <br>
                <!-- footerBottomSection -->	
                &copy; 2014, Allright reserved.
                <div class="pull-right">Powered By<a href="#" target="_blank" title="OceanBrains" class="btn-link">&nbsp;OceanBrains.</a></div>
            
            </div>
	</div>
	
<!-- JS Global Compulsory -->			
<script type="text/javascript" src="assets/custom/js/jquery-1.8.2.min.js"></script>
<script type="text/javascript" src="assets/custom/js/modernizr.custom.js"></script>		
<script type="text/javascript" src="assets/bootstrap/js/bootstrap.min.js"></script>	
	
<!-- JS Implementing Plugins -->
<script type="text/javascript" src="assets/custom/js/jquery.flexslider-min.js"></script>
<script type="text/javascript" src="assets/custom/js/modernizr.js"></script>
<script type="text/javascript" src="assets/custom/js/jquery.cslider.js"></script> 
<script type="text/javascript" src="assets/custom/js/back-to-top.js"></script>
<script type="text/javascript" src="assets/custom/js/jquery.sticky.js"></script>

<!-- JS Page Level -->           
<script type="text/javascript" src="assets/custom/js/app.js"></script>
<script type="text/javascript" src="assets/custom/js/index.js"></script>


<script type="text/javascript">
    jQuery(document).ready(function() {
      	App.init();
        App.initSliders();
        Index.initParallaxSlider();
    });
</script>


</body>
</html>